...
findEscapesT escaper =
  let initialState = EscaperState { depth = 0, env = Map.empty, astPath = [] }
  in  runIdentity (evalStateT (execWriterT escaper) initialState)

findEscapes :: A.Exp -> [(Symbol, AstPath)]
findEscapes exp = toList $ findEscapesT $ findEscapesM exp
...
pushDir
  ...
  => EscaperState
  -> AstDir
  -> t (StateT EscaperState m) ()
pushDir state dir = lift $ put state { astPath = astPath state ++ [dir] }

pushBinding
  :: (MonadTrans t, Monad m, Monad (t (StateT EscaperState m)))
  => Symbol
  -> BindingKind
  -> t (StateT EscaperState m) ()
...
extendEnvWithFunctionDec fundecs = do
 ...
 where
  ...
  processBodies paramsAndExps =
    forM_ (enumerate paramsAndExps) processBodiesMapFun
  processBodiesMapFun :: (Int, ([A.Field], A.Exp)) -> Escaper ()
  processBodiesMapFun (fundecIdx, (params, bodyExp)) = do
    state <- lift get
    pushDir state (FunDec fundecIdx)
    forM_ params extendByParams
    state' <- lift get
...
escapePaths = foldl' escapeExpPath

escapeExp exp = escapePaths exp $ findEscapes exp

escapeExpPath :: A.Exp -> (Symbol, AstPath) -> A.Exp
escapeExpPath callExp@(A.CallExp _ args _) (sym, CallArg idx : path') = callExp
  { A.args = replaceNth idx args (escapeExpPath (args !! idx) (sym, path'))
  }
...
escapeExpPath recordExp@(A.RecordExp fields _ _) (sym, RecField idx : path') =
  let (sym', exp, pos) = fields !! idx
      exp'             = escapeExpPath exp (sym, path')
  in  recordExp { A.fields = replaceNth idx fields (sym', exp', pos) }
escapeExpPath (A.SeqExp seqElts) (sym, SeqElt idx : path') =
  let (exp, pos) = seqElts !! idx
      exp'       = escapeExpPath exp (sym, path')
  in  A.SeqExp $ replaceNth idx seqElts (exp', pos)
...
escapeExpPath forExp@A.ForExp{} (_, []) = forExp { A.escape = True }
...
escapeExpPath letExp@A.LetExp { A.decs = decs } (sym, []) = letExp
  { A.decs = escapeVarDec sym decs
  }
 where
  escapeVarDec :: Symbol -> [A.Dec] -> [A.Dec]
  escapeVarDec s ds =
    let Just idx = findIndex
          (\dec -> case dec of
          ...
          ds
        varDec = ds !! idx
    in  replaceNth idx ds $ varDec { A.vardecEscape = True }
escapeExpPath letExp@(A.LetExp decs _ _) (sym, LetDec idx : path') = letExp
  { A.decs = replaceNth idx decs (escapeDecPath (decs !! idx) (sym, path'))
  }
...
escapeExpPath expr path' =
  error
    $  "shouldn't get here: expr = "
    ++ show expr
    ++ "\npath = "
    ++ show path'
...
escapeDecPath (A.FunctionDec funDecs) (sym, [FunDec funIdx]) =
  A.FunctionDec
    $ replaceNth funIdx funDecs (escapeParam (funDecs !! funIdx) sym)
 where
  escapeParam :: A.FunDec -> Symbol -> A.FunDec
  escapeParam funDec@(A.FunDec _ params _ _ _) s =
    let Just idx = elemIndex s $ fmap A.fieldName params
    in  funDec { A.params = replaceNth idx params (escapeField $ params !! idx)
               }
  escapeField :: A.Field -> A.Field
  escapeField field@A.Field{} = field { A.fieldEscape = True }
escapeDecPath (A.FunctionDec funDecs) (sym, FunDec funIdx : FunBody : path') =
  let funDec  = funDecs !! funIdx
      funBody = A.funBody funDec
  in  A.FunctionDec $ replaceNth
        funIdx
        funDecs
        funDec { A.funBody = escapeExpPath funBody (sym, path') }
escapeDecPath varDec@A.VarDec{} (_, []) = varDec { A.vardecEscape = True }
escapeDecPath varDec@A.VarDec { A.decInit = initExp } (sym, DecInit : path') =
  varDec { A.decInit = escapeExpPath initExp (sym, path') }
escapeDecPath dec path' =
  error $ "shouldn't get here: dec = " ++ show dec ++ "\npath = " ++ show path'
...
getEscape (A.RecordExp fields _ _) (sym, RecField idx : path') =
  let (_, exp, _) = fields !! idx in getEscape exp (sym, path')
getEscape (A.SeqExp seqElts) (sym, SeqElt idx : path') =
  let (exp, _) = seqElts !! idx in getEscape exp (sym, path')
...
getEscape A.LetExp { A.decs = decs } (sym, []) =
  let Just A.VarDec { A.vardecEscape = escape } = vardecWithSym in escape
 where
