...
linearize stm = runState (linearizeM stm)
 where
  ...
  reorderExp
    :: [T.Exp] -> ([T.Exp] -> T.Exp) -> State Temp.Generator (T.Stm, T.Exp)
  ...
  doStm (T.EXP (T.CALL (e, el, escapes, hasRet))) = reorderStm (e : el)
    $ \(e' : el') -> T.EXP $ T.CALL (e', el', escapes, hasRet)
...
traceSchedule blocks done = do
 ...
 where
    ...
    in  case splitLast b of
          ...
          (most, T.CJUMP (op, x, y, t, f)) ->
            case (Map.lookup t table, Map.lookup f table) of
              ...
              (Just b'@(_ : _), _) -> do
                tl <- trace table b' rest
                pure $ most ++ [T.CJUMP (T.notRel op, x, y, f, t)] ++ tl
              _ -> do
                f'   <- newLabel
                next <- getNext table rest
                pure
  ...
  splitLast :: [a] -> ([a], a)
  splitLast [x    ] = ([], x)
  splitLast (h : t) = let (t', l) = splitLast t in (h : t', l)
