    ...
    do
      ...
      case n of
        ...
        5 -> do
          ...
          pure $ T.CALL
            (funcExp, args, fmap (const Frame.DoesNotEscape) args, hasResult)
        _ -> do
          s <- stmGen
          e <- expGen
          pure $ T.ESEQ (s, e)
...
instance Arbitrary T.Relop where
  arbitrary = elements
    [T.EQ, T.NE, T.LT, T.GT, T.LE, T.GE, T.LE, T.GE, T.ULT, T.ULE, T.UGT, T.UGE]
...
prop_canonHasNoSeq stm =
  let (stms, _) = C.linearize stm gen in all stmHasNoSeq stms
 where
...
prop_parentOfCallIsOk stm =
  let (stms, _) = C.linearize stm gen in all callParentOkStm stms
 where
  ...
  callParentOkExp (T.BINOP (_, e1, e2)) =
    callParentOkExp e1 && callParentOkExp e2
  ...
  callParentOkStm (T.CJUMP (_, e1, e2, _, _)) =
    callParentOkExp e1 && callParentOkExp e2
...
main :: IO ()
main = hspec $ do
  describe "linearize" $ it "has no SEQ nor ESEQ" $ property prop_canonHasNoSeq
  describe "linearize"
