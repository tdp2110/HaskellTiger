...
parse text = let (Right ast) = Parser.parse text in ast
...
test_directions1 = TestCase
  (let ast     = parse text1
   ...
   in  assertEqual "directions1" [(Symbol "y", [LetDec 0, FunDec 0])] escapes
  )
...
test_subscript = TestCase
  (let text =
   ...
   in  assertEqual "subscript" [(Symbol "N", []), (Symbol "arr", [])] escapes
  )
...
test_assign = TestCase
  (let text =
           "let\n"
             ...
             ++ "end"
       escapes = findEscapes $ parse text
   in  assertEqual "assign" [(Symbol "x", [])] escapes
  )
